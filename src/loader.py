...
def load_statement(file_path: str | Path) -> pd.DataFrame:
    """
    ...
    if suffix in {".xlsx", ".xls"}:
        df = _load_excel_frame(file_path)
    elif suffix == ".csv":
        df = _load_csv_frame(file_path)
    else:
        df = _load_pdf_frame(file_path)
...
def _load_excel_frame(file_path: Path) -> pd.DataFrame:
...
def _load_pdf_frame(file_path: Path) -> pd.DataFrame:
    try:
        import pdfplumber
